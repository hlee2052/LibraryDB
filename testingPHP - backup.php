...
if ($db_conn) {
	...
	if (array_key_exists('reset', $_POST)) {
		...
		executePlainSQL("create table tab1 (nid number, name varchar2(30), address varchar2(30), primary key (nid))");
		OCICommit($db_conn);
		...
		if (array_key_exists('insertsubmit', $_POST)) {
			...
			executeBoundSQL("insert into Staff1 values (:bind1, :bind2)", $alltuples);
			OCICommit($db_conn);
			...
			// this is where you can update new address		 
			 else if (array_key_exists('updatesubmit1', $_POST)) {
				// Update tuple using data from user
				$tuple = array (
					":bind1" => $_POST['oldName1'],
					":bind2" => $_POST['newName1']
				);
				$alltuples = array (
					$tuple
				);
				executeBoundSQL("update tab1 set address=:bind2 where address=:bind1", $alltuples);
				OCICommit($db_conn);

			} 
			
			
			       // deleting rows with certain address 
				...
				executeBoundSQL("delete from tab1 where ADDRESS=:bind1", $alltuples);
				OCICommit($db_conn);
				...
				if (array_key_exists('dostuff', $_POST)) {
					...
					executePlainSQL("insert into tab1 values (10, 'Frank', '1234 House Ave')");
...
$stid = oci_parse($conn, 'SELECT * FROM Staff1');
if (!$stid) {
    $e = oci_error($conn);
    trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
}
